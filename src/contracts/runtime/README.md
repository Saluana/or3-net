...
This package does not replace the existing node protocol or provider SDKs.

Instead, it wraps the current execution infrastructure behind a uniform contract:

- `OpenSandboxRuntimeAdapter` wraps the built-in OpenSandbox provider integration
